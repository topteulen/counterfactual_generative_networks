...
class TensorDataset2(Dataset):
    ...
    def __getitem__(self, index):
        i, j = divmod(index, self.batch_size)
        return self.data[i][j], self.labels[index]
...
class DoubleColoredMNIST(Dataset):
    def __init__(self, train=True, counterfactual=False, rotate=0, translate=None, scale=None, shear=None):
        ...
        def get_rgb(x):
            return np.array(ImageColor.getcolor(x, "RGB"))/255.

        background_colors = np.array(list(map(get_rgb, colors1)))
        object_colors = np.array(list(map(get_rgb, colors2)))

        # create color noise for all images
        ...
        back_colors += torch.tensor(background_colors[labels]).reshape(-1, 3, 1, 1)
        obj_colors += torch.tensor(object_colors[labels]).reshape(-1, 3, 1, 1)
...
def get_tensor_dataloaders(dataset, batch_size=64):
    ...
    if 'counterfactual' in dataset:
        ...
        if isinstance(tensor[0], list):
            ds_train = TensorDataset2(tensor[0], tensor[1])
        else:
            ds_train = TensorDataset(tensor[0], tensor[1])
